...
#
# Copyright (c) 2022-2025 Lin Song <linsongui@gmail.com> <3
...
disable_client() {
    ...
    wg set wg0 peer "$(sed -n "/^# BEGIN_PEER $client$/,\$p" "$WG_CONF" | grep -m 1 PublicKey | cut -d " " -f 3)" allowed-ips ""

    # Update config file by commenting out AllowedIPs ( -i flag for edit)
	sed -i "/^# BEGIN_PEER $client$/,/^# END_PEER $client$/s/^\(AllowedIPs.*\)$/# \1/" "$WG_CONF"

	echo "Client '$client' has been disabled."
}
...
if [[ ! -e "$WG_CONF" ]]; then
	...
	case "$option" in
		...
		7)
			check_clients
			select_client_to "Enable client"
			enable_client
			exit 0
		;;
		8)
