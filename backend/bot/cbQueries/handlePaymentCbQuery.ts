...
export default async function handlePaymentCbQuery(ctx: Context) {
    if (!ctx.from) {
        return reportError({}, "Missing ctx.from");
    }

    if (!ctx.callbackQuery) {
        return reportError({}, "Missing ctx.callbackQuery");
    }
    ...
    if (!rawData) {
        return reportError({}, "Missing ctx.callbackQuery.data: " + ctx.callbackQuery.data);
    }
